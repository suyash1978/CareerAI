      ...
      <div className="glass-panel w-full max-w-3xl rounded-3xl border border-slate-800 p-6 sm:p-8 my-8 shadow-2xl relative space-y-6">
          ...
          <div className="glass-card p-5 rounded-2xl border border-slate-800 space-y-4 text-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="glass-card p-3 rounded-xl border border-slate-800/80 space-y-1">
                ...
                <span className="text-sm font-extrabold text-blue-400">{subScores.skills_score} / 50</span>
              </div>

              <div className="glass-card p-3 rounded-xl border border-slate-800/80 space-y-1">
                <span className="text-slate-400 block">Experience Score</span>
                <span className="text-sm font-extrabold text-purple-400">{subScores.experience_score} / 30</span>
              </div>

              <div className="glass-card p-3 rounded-xl border border-slate-800/80 space-y-1">
                <span className="text-slate-400 block">Education Score</span>
                <span className="text-sm font-extrabold text-emerald-400">{subScores.education_score} / 20</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
              {matchingSkills.length > 0 && (
                <div>
                  <span className="text-emerald-400 font-semibold block mb-1">Matching Job Skills:</span>
                  <div className="flex flex-wrap gap-1">
                    {matchingSkills.map((s, idx) => (
                      <span key={idx} className="text-[11px] px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 font-medium">
                        ✓ {s}
                      </span>
                    ))}
                  </div>
              ...
              {missingSkills.length > 0 && (
                <div>
                  <span className="text-amber-400 font-semibold block mb-1">Missing Target Skills:</span>
                  <div className="flex flex-wrap gap-1">
                    {missingSkills.map((s, idx) => (
                      <span key={idx} className="text-[11px] px-2 py-0.5 rounded bg-amber-500/10 text-amber-300 border border-amber-500/20 font-medium">
                        + {s}
                      </span>
                    ))}
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
          ...
          <div className="glass-card p-4 rounded-2xl border border-slate-800 space-y-3">
            <h4 className="font-bold text-white flex items-center space-x-2 border-b border-slate-800 pb-2">
        ...
        {resumeUrl && (
          <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <span className="text-xs font-bold text-white block">Submitted PDF Resume</span>
            ...
            <a
              ...
              className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs flex items-center space-x-1.5 transition-all shadow-lg shadow-blue-600/20"
            >
        ...
        <form onSubmit={handleSaveStatus} className="glass-card p-5 rounded-2xl border border-slate-800 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h4 className="font-bold text-white text-xs">Recruiter Decision & Evaluation Notes</h4>
            {successMsg && (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="sm:col-span-1 space-y-1.5">
              <label className="text-[11px] font-semibold text-slate-400 block">Candidate Status</label>
              <select
                value={status}
            ...
            <div className="sm:col-span-2 space-y-1.5">
              <label className="text-[11px] font-semibold text-slate-400 block">Recruiter Evaluation Notes</label>
              <input
                type="text"
                value={recruiterNotes}
                onChange={(e) => setRecruiterNotes(e.target.value)}
                placeholder="Add confidential notes on interview feedback, technical assessment, or salary expectations..."
                className="w-full bg-slate-900 border border-slate-700/80 rounded-xl px-3 py-2 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500"
              />
            </div>
          </div>
          ...
          <div className="flex justify-end pt-1">
            <button
              type="submit"
              disabled={saving}
              className="px-5 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-semibold text-xs flex items-center space-x-1.5 shadow-lg shadow-purple-600/20 transition-all"
            >
