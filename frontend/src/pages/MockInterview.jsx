    ...
    <div className="space-y-8 py-4">
      ...
      <div className="glass-panel p-8 rounded-3xl border border-slate-800 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center space-x-4">
            <div className="p-4 rounded-2xl bg-gradient-to-tr from-indigo-600 to-blue-600 text-white shadow-lg shadow-indigo-500/20">
              <MessageSquare className="w-8 h-8" />
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <h1 className="text-2xl font-bold text-white">AI Mock Interview Simulator</h1>
          ...
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 hover:text-white hover:border-slate-700 text-xs font-semibold transition-all self-start md:self-auto"
          >
      ...
      {error && (
        <div className="p-4 rounded-2xl bg-rose-500/10 border border-rose-500/30 text-rose-400 text-xs flex items-center justify-between">
          ...
          <button onClick={() => setError('')} className="underline font-semibold">
            Dismiss
          </button>
        </div>
      )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-xs">
              {history.map((s) => (
                <div
                  ...
                  className="glass-card p-4 rounded-2xl border border-slate-800 hover:border-indigo-500/40 cursor-pointer space-y-2 group transition-all"
                >
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-white group-hover:text-indigo-300">{s.target_role}</span>
                  ...
                  <div className="text-[11px] text-slate-400 space-y-0.5">
                    ...
                    <p className="text-slate-500">{s.created_at}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
          ...
          <form onSubmit={handleStartInterview} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 block">Target Job Title / Role</label>
              <input
                type="text"
                required
                value={targetRole}
                ...
                className="w-full bg-slate-900 border border-slate-700/80 rounded-2xl px-4 py-3 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500"
              />

              <div className="flex flex-wrap gap-2 pt-1">
                <span className="text-[11px] text-slate-500 font-semibold self-center">Presets:</span>
                {PRESET_ROLES.map((r) => (
                  <button
                    key={r}
                    type="button"
                    onClick={() => setTargetRole(r)}
                    className="text-[11px] px-2.5 py-1 rounded-xl bg-slate-900 border border-slate-800 text-slate-400 hover:text-white"
                  >
                    {r}
                  </button>
                ))}
            ...
            {/* Experience Level */}
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 block">Target Seniority Level</label>
            ...
            {/* Target Technologies */}
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 block">Required Technologies & Skills</label>
              <input
                type="text"
                value={technologies}
            ...
            {activeJobs.length > 0 && (
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-300 block">Align with Active Job Posting (Optional)</label>
                <select
                  value={selectedJobId}
                  onChange={(e) => {
                  ...
                  className="w-full bg-slate-900 border border-slate-700/80 rounded-2xl px-4 py-3 text-xs text-slate-200 focus:outline-none focus:border-indigo-500"
                >
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold text-sm shadow-xl shadow-indigo-600/30 flex items-center justify-center space-x-2 transition-all"
            >
              {loading ? (
                <>
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="text-slate-400 font-semibold">{sessionData.target_role}</span>
            </div>
          ...
          {(() => {
            ...
            return (
              <div className="glass-card p-6 rounded-3xl border border-slate-800 space-y-4">
                <div className="flex items-center justify-between">
                  <span className={`text-xs font-bold px-2.5 py-0.5 rounded border ${getQuestionBadgeColor(q.question_type)}`}>
                    {q.question_type} QUESTION
                  </span>

                  <span className="text-[11px] font-bold px-2 py-0.5 rounded bg-slate-900 border border-slate-800 text-slate-400">
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <textarea
                    ...
                    className="w-full bg-slate-900 border border-slate-800 rounded-2xl p-4 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 leading-relaxed"
                  />
                </div>
          ...
          <div className="flex items-center justify-between pt-2">
            <button
              ...
              className={`px-4 py-2.5 rounded-xl text-xs font-semibold flex items-center space-x-1 transition-all ${
            ...
            {currentQuestionIdx < sessionData.questions.length - 1 ? (
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs flex items-center space-x-1 transition-all shadow-lg shadow-indigo-600/20"
              >
              ...
              <button
                ...
                className="px-6 py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center space-x-2 shadow-xl shadow-emerald-600/30 transition-all"
              >
                {loading ? (
                  <>
        ...
        <div className="space-y-8">
          ...
          <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-800 grid grid-cols-1 md:grid-cols-4 gap-6 items-center">
            ...
            <div className="md:col-span-1 flex flex-col items-center justify-center text-center p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2">
              <span className="text-xs font-semibold text-slate-400">Overall Interview Score</span>
            ...
            <div className="md:col-span-3 space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <h2 className="text-lg font-bold text-white">AI Evaluation Scorecard</h2>
                </div>

                <button
                  onClick={() => { setMode('SETUP'); setEvaluation(null); }}
                  className="flex items-center space-x-1.5 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-xs font-semibold text-slate-300 hover:text-white transition-all"
                >
              ...
              {evaluation.questions.map((q, idx) => (
                <div key={q.id || idx} className="glass-card p-6 rounded-3xl border border-slate-800 space-y-4">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
                    <div className="flex items-center space-x-2">
                      <span className={`text-[11px] font-bold px-2 py-0.5 rounded border ${getQuestionBadgeColor(q.question_type)}`}>
                  ...
                  <div className="p-3.5 rounded-2xl bg-slate-900/80 border border-slate-800 text-xs space-y-1">
                    <span className="text-slate-500 font-semibold block text-[11px]">YOUR SUBMITTED RESPONSE:</span>
                    <p className="text-slate-300 leading-relaxed whitespace-pre-line">
                      {q.user_answer || <em className="text-slate-500">No response submitted.</em>}
                    </p>
                  </div>
